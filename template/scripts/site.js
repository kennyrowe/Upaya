Y.use('node', function (Y) {
	window.Site = Singleton.create({
		...
		init: function() {
			...
			if (!this._touch) {
				Y.later(200, this, function () {
					this.setScrolledClass('#preFooter');
				});
			}
		...
		setScrolledClass: function (element){

			if (Y.one(element)) {
				Y.all(element).each(function (e){
					if (e.getY() > Y.one('body').get('winHeight') + window.scrollY) {
						e.addClass('unscrolled');
					}
				});
			}
		...
		setActiveCategory: function(){
			...
			var categoryNav = Y.one('#categoryNav');
			var categoryLinks = Y.all('.category-nav-links li a');

			if (categoryNav) {
				var category = location.search.split('=')[1];
				if (category) {
					var categoryFound = false;
					for (i=0; i<categoryLinks.size() && categoryFound===false; i++) {
						var currentCategoryLink = categoryLinks.item(i);

						if (encodeURI(currentCategoryLink.getAttribute('href').split('=')[1]) == category) {
							categoryFound = true;
							currentCategoryLink.get('parentNode').addClass('active-link');
						}
					}
				} else {
					Y.one('.category-nav-links li:not(.nav-section-label)').addClass('active-link');
				}
			}
